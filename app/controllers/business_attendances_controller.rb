...
class BusinessAttendancesController < ApplicationController
  ...
  def index
    ...
    if params[:q].present? && params[:q].present?
      @business_attendances = @business_attendances.where("CONCAT_WS(' ', first_name, last_name) LIKE :search OR first_name LIKE :search OR last_name LIKE :search OR email LIKE :search OR name LIKE :search OR org_name LIKE :search",
                                                          search: "%#{params[:q]}%"
                                                         )
    end
  end
